...
import type {MockSyncExecutionContext} from 'coda-packs-sdk/dist/development';
...
import {executeMetadataFormula} from 'coda-packs-sdk/dist/development';
...
describe('Github pack', () => {
  describe('ReviewPullRequest', () => {
    ...
    it('executes ReviewPullRequest', async () => {
      // Create a fake response body, using the type we defined as a guide for what fields should be included.
      const fakeReviewResponse: PullRequestReviewResponse = {
        id: 789,
        user: {
          id: 456,
      ...
      // Check the actual response matches what we'd expect from our fake response after it goes through normalization.
      // Since this is an action formula, the response is not nearly as interesting as the request itself, which
      // we verify below.
      ...
      // Ensure that the API call we made to post a review comment was constructed properly.
      sinon.assert.calledOnceWithExactly(context.fetcher.fetch, {
        ...
        body: JSON.stringify({body: 'review body', event: GitHubReviewEvent.RequestChanges}),
      });
    });

    it('comment request requires comment body', async () => {
      // Our formula implementation has custom validation that if you use a Comment action, that
  ...
  describe('PullRequests sync table', () => {
    ...
    beforeEach(() => {
      ...
      // This will allow us to register fake fetcher responses.
      // This context object is slightly different than a MockExectionContext that we use with non-sync formuls.
      syncContext = newMockSyncExecutionContext();
    });

    // A helper function to create valid dummy data, since the GitHub objects are quite large.
    // This allows us to specify a handful of specific fields of interest and populate the rest with defaults.
    function makeFakePullRequest(overrides: Partial<GitHubPullRequest>): GitHubPullRequest {
      ...
      const defaults: GitHubPullRequest = {
        ...
        assignees: [],
        requested_reviewers: [],
        requested_teams: [{id: 765, name: 'some team', html_url: 'https://some-team-url.com'}],
      };
      return {...defaults, ...overrides};
    }

    // A helper function to generate the http header that GitHub uses to tell us that there is a
    // next page or previous page of results.
    ...
    // A simple test to make sure all the plumbing works, that doesn't deal with pagination.
    it('syncs a single page', async () => {
      const pr1 = makeFakePullRequest({title: 'pull request 1', number: 111});
      const pr2 = makeFakePullRequest({title: 'pull request 2', number: 222});
      ...
      const syncedObjects = await executeSyncFormulaFromPackDef(
        manifest,
        'PullRequests',
      ...
      // Make sure we only made one http request to get results, and that the url
      // was what we expected it to me.
    ...
    it('syncs multiple pages', async () => {
      const pr1 = makeFakePullRequest({title: 'pull request 1', number: 111});
      const pr2 = makeFakePullRequest({title: 'pull request 2', number: 222});
      // Create a response to our first http request, that returns a PR but includes an http
      // header that tells us there is a next page of results.
      const page1Response = newJsonFetchResponse([pr1], 200, {
        link: makeLinkHeader({next: 'https://api.github.com/next-page-url'}),
      });
      // Create a response for our second http request, that doesn't have a next page header,
      // which means our sync should terminate because there aren't any more pages to request.
      const page2Response = newJsonFetchResponse([pr2]);
      // Register these responses with our mock fetcher. The mock fetcher will check the incoming arguments
      // and return the appropriate one of its multiple registered responses.
      ...
      // Now actually execute the sync. This will keep fetching additional pages of results until there
      // is not continuation returned.
      const syncedObjects = await executeSyncFormulaFromPackDef(
        manifest,
        'PullRequests',
        ['https://github.com/some-org/some-repo'],
      ...
      // Make sure the results from each page got concatenated together into one big list of results.
      assert.equal(syncedObjects.length, 2);
      assert.equal(syncedObjects[0].Title, 'pull request 1');
      assert.equal(syncedObjects[1].Title, 'pull request 2');
    ...
    // We can even test the metadata formulas in the supporting parts of our pack.=
    // The repo url parameter to this sync table has an autocomplete metadata formula to make it easier
    // for users to select the repo they want to sync from. This test exercises that formula,
    // which makes http requests to GitHub to listen repos that the user has access to.
    it('repo url autocomplete', async () => {
      // Create a mock execution context, some fake repos, and register an http response that returns those repos.
      const context: MockExecutionContext = newMockExecutionContext();
      ...
      const repo2: GitHubRepo = {...repo1, id: 456, name: 'repo 2 matches some-query', html_url: 'https://repo-2-url'};
      context.fetcher.fetch.returns(newJsonFetchResponse([repo1, repo2]));

      // Invoke the metadata formula simulating that the user has searched in the UI for 'some-query'.
      const results = await executeMetadataFormula(repoUrlParameter.autocomplete!, {search: 'some-query'}, context);

      // The metadata formula should fetch our fake repos but filter them to only those that match the search query.
