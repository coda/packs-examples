...
describe("Trigonometry pack", () => {
  it("executes Cosine", async () => {
    ...
    assert.approximately(
      1 / Math.sqrt(2),
      await executeFormulaFromPackDef<number>(manifest, "Cosine", [
        Math.PI / 4,
      ]),
      1e6,
    );
  ...
  it("converts degrees and radians", async () => {
    assert.equal(
      Math.PI / 2,
      await executeFormulaFromPackDef(manifest, "ToRadians", [90]),
    );
    assert.equal(
      90,
      await executeFormulaFromPackDef(manifest, "ToDegrees", [Math.PI / 2]),
    );
  });
});
